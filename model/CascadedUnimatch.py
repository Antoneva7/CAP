...
def get_vessel_bbox(
    ...
    # 取血管区域（class=2）
    vessel_bin = (vessel_mask == 2).float()  # [B,H,W]
...
class VesselUNet(nn.Module):
    ...
    """
    def __init__(self, in_chns: int = 1, seg_cls: int = 3, base_ch: int = 32):
        super().__init__()
        bc = base_ch   # 32
...
class CascadedUniMatch(nn.Module):
    ...
    def _merge_seg(
        ...
        """
        融合Stage1（血管）和Stage2（斑块）的分割输出。

        策略：
        - class 0 (背景): 取两者背景logit的平均
        - class 1 (斑块): 优先使用Stage2（斑块专家）的输出
        - class 2 (血管): 优先使用Stage1（血管专家）的输出

        Args:
            vessel_logits : [B, seg_cls, H, W]  Stage1输出
            plaque_logits : [B, seg_cls, H, W]  Stage2输出

        Returns:
            merged : [B, seg_cls, H, W]
        """
        # 背景：两者平均更稳定
        bg    = (vessel_logits[:, 0:1] + plaque_logits[:, 0:1]) / 2.0
        # 斑块：Stage2主导（0.7），Stage1辅助（0.3）
        plq   = plaque_logits[:, 1:2] * 0.7 + vessel_logits[:, 1:2] * 0.3
        # 血管：Stage1主导（0.7），Stage2辅助（0.3）
        vsl   = vessel_logits[:, 2:3] * 0.7 + plaque_logits[:, 2:3] * 0.3

        return torch.cat([bg, plq, vsl], dim=1)   # [B, 3, H, W]
